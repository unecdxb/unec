...
const ProjectList = ({ data, regionData, categoryData }: { data: any, regionData: { name: string }[], categoryData: { name: string }[] }) => {
    ...
    return data.projects.filter((item: any) => {
      ...
      const keywordMatch = appliedKeyword.trim() === "" ||
        [item.title, item.firstSection.category?.name, item.firstSection.location?.name]
          .filter(Boolean)
          .some((field: string) => field.toLowerCase().includes(appliedKeyword.toLowerCase()));
  ...
  useEffect(() => {
    ...
    const items = gsap.utils.toArray(gridRef.current.children);
    const ctx = gsap.context(() => {
      gsap.fromTo(items, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: 0.5, stagger: 0.05, ease: "power2.out" });
    }, gridRef);
  ...
  return (
    <section className="sp-py">
      <div className="container">
        ...
        {!isDesktop && (
          <motion.div variants={moveUp(0.2)} initial="hidden" whileInView="show" viewport={{ amount: 0.1, once: true }} className="mb-6 xl:mb-12">
            <button onClick={() => setOpenFilters(prev => !prev)} className="w-full h-14 px-6 border border-black flex items-center justify-between uppercase tracking-[0.15em] text-sm font-semibold group transition-colors duration-300 hover:bg-black hover:text-white">
              <span>Filter</span>
              <motion.span animate={{ rotate: openFilters ? 180 : 0 }} transition={{ duration: 0.3 }} className="text-3xl text-black leading-none">
                {openFilters ? "–" : "+"}
              </motion.span>
            </button>
          </motion.div>
        ...
        >
          <div className="bg-light p-4 xl:p-8 mb-8 xl:mb-12">
            ...
            >
              ...
              <motion.div variants={{ hidden: { y: 15, opacity: 0 }, show: { y: 0, opacity: 1 } }}>
                <Select inputId="region-select" options={regions} styles={selectPortalStyles}
menuPortalTarget={typeof window !== "undefined" ? document.body : null}
menuPosition="fixed"
 value={region} defaultValue={regions[0]} onChange={setRegion} classNamePrefix="cmn-select" className="cmn-select" instanceId="region-select" />
              </motion.div>

              <motion.div variants={{ hidden: { y: 15, opacity: 0 }, show: { y: 0, opacity: 1 } }}>
                <Select inputId="category-select" options={categories} styles={selectPortalStyles}
menuPortalTarget={typeof window !== "undefined" ? document.body : null}
menuPosition="fixed"
 value={category} defaultValue={categories[0]} onChange={setCategory} classNamePrefix="cmn-select" className="cmn-select" instanceId="category-select" />
              </motion.div>

              <motion.div variants={{ hidden: { y: 15, opacity: 0 }, show: { y: 0, opacity: 1 } }}>
                <Select inputId="status-select" options={statuses} styles={selectPortalStyles}
menuPortalTarget={typeof window !== "undefined" ? document.body : null}
menuPosition="fixed"
 value={status} defaultValue={statuses[0]} onChange={setStatus} classNamePrefix="cmn-select" className="cmn-select" instanceId="status-select" />
              </motion.div>

              <motion.div variants={{ hidden: { y: 15, opacity: 0 }, show: { y: 0, opacity: 1 } }}>
                <input id="keyword-input" type="text" placeholder="Type keyword…" className="w-full h-[48px] pr-4 border-b border-black/20 focus:outline-none focus:border-black placeholder:text-black" value={keyword} onChange={e => setKeyword(e.target.value)} />
              </motion.div>

              <motion.button variants={{ hidden: { y: 15, opacity: 0 }, show: { y: 0, opacity: 1 } }} className="h-12 px-8 cursor-pointer bg-black text-white flex items-center justify-center gap-2.5 font-semibold text-sm tracking-[0.15em] uppercase relative overflow-hidden group border-2 border-white" onClick={handleSearch}>
                <span className="absolute inset-0 bg-white transform translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out"></span>
                <FiSearch className="w-4 h-4 stroke-[2.5] relative z-10 group-hover:scale-110 transition-all duration-500 group-hover:text-black" />
                <span className="relative z-10 group-hover:text-black transition-colors duration-500">Search</span>
              </motion.button>
        ...
        {filteredItems.length === 0 ? (
          ...
          <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredItems.map((item: any, index: number) => (
              <Link href={`/projects/${item.slug}`} key={index}>
                <motion.div variants={moveUp( 0.1 * index)} initial="hidden" whileInView="show" viewport={{ amount: 0.1, once: true }}>
                  <ProjectCard image={item.thumbnail} title={item.title} location={item.firstSection.location.name} height="h-[250px] xl:h-[350px]" />
                </motion.div>
              </Link>
            ))}
