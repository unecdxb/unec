...
const SubContents = ({ data }: { data: CareerData['firstSection']['items'] }) => {
  return (
    <section className="sp-py" id="associated-business">
      <div className="container">
        {data.map((item, index) => (
          <div key={index}>
            <hr className={index === 0 ? "hidden" : "h-[1px] border-gray-200 mb-0"} />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 xl:gap-20 items-center bg-light px-6 py-8 xl:px-12 xl:py-15">
              ...
              <div className={`${index % 2 === 0 ? "xl:order-1" : "xl:order-2"}`}>
                <motion.h3 variants={moveUp(0.2)} initial="hidden" whileInView="show" viewport={{ amount: 0.1, once: true }} className="text-32 uppercase text-secondary font-normal mb-2 xl:mb-4">{item.title}</motion.h3>
                <div className="space-y-4">
                  {item.description.split("\n").map((item, index) => (
                    <motion.p variants={moveUp(0.4)} initial="hidden" whileInView="show" viewport={{ amount: 0.1, once: true }} key={index}>{item}</motion.p>
                  ))}
