        ...
        <div className="grid grid-cols-1 md:grid-cols-2  gap-y-10 gap-x-10 ">
          <div className="flex flex-col justify-between gap-4">
            ...
            <motion.div variants={moveUp(0.6)} initial="hidden" whileInView="show" viewport={{ amount: 0.1, once: true }}>
              <SecondaryBtn href={`/pdf-viewer?url=${data.file}`} target="_blank" text={data.fileTitle} />
            </motion.div>
          </div>
          <div>
            <div className="grid grid-cols-2 xs:grid-cols-3  md:grid-cols-2 gap-[2px]">
              {data.items.map((stat, index) => (
                <MechanicalCounter key={index} keyIndex={index} end={Number(stat.number)} label={stat.value} suffix={''} duration={2.5} />
              ))}
            </div>
          </div>
        </div>
