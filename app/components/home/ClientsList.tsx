            ...
            data.items.map((client, index) => (
              <div key={index} className="w-full border-r border-b border-black/10 flex items-center justify-center overflow-hidden group">
                <motion.div variants={zoomIn(0.3 + index * 0.1)} initial="hidden" whileInView="show" viewport={{ amount: 0.1, once: true }}>
                  <Image src={client.logo} alt={client.logoAlt} width={350} height={218} className="w-[250px] h-[120px] xl:w-[280px] xl:h-[200px] object-contain lg:group-hover:scale-110 transition-all duration-300 filter opacity-50 grayscale hover:opacity-100 hover:grayscale-0" />
                </motion.div>
              </div>
            ))
          }
