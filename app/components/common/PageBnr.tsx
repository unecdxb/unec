...
const PageBnr = ({ title, image, imageAlt }: PageBnrProps) => {
  return (
    <section className="h-[350px] md:h-[300px] lg:h-[350px] xl:h-[450px] bg-black/60 w-full relative">
      ...
      {image && (
        <Image src={image} alt={imageAlt || "Banner"} className="w-full h-full object-cover absolute top-0 left-0 z-0" width={1920} height={1080} />
      )}
      <div className="container h-full relative z-10">
        <div className="flex flex-col justify-end gap-4 h-full pb-10 md:pb-16 lg:pb-20 xl:pb-24">
          <motion.h1 variants={moveRight(0.2)} initial="hidden" animate="show" viewport={{ amount: 0.1, once: true }} className="text-25 font-bold uppercase text-white leading-1.1">{title}</motion.h1>
          <Breadcrumb />
        </div>
      </div>
    </section>
