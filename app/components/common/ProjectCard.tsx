...
const ProjectCard = ({image,title,location,height}:ProjectCardProps) => {
  return (
    <div className={`group relative flex flex-col justify-end overflow-hidden  ${height ? height :"h-[250px] xl:h-[300px]"}`}>
      ...
      <div className="absolute inset-0 z-10 bg-gradient-to-t from-black/90 via-black/30 to-transparent xl:hidden"></div>
      <div className="max-xl:absolute max-xl:opacity-100 max-xl:inset-0 max-xl:flex max-xl:flex-col max-xl:justify-end relative p-4 xl:p-5 opacity-0 group-hover:opacity-100 xl:bg-black/70  transition-all duration-500 z-10">
