    ...
    return (
        ...
        <div className="border-t border-gray-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 py-6 border-b border-gray-300">
            ...
            <section className="w-full mx-auto py-2">
                <h2 className="text-2xl font-semibold tracking-wide mb-10 uppercase">PROJECT DETAILS</h2>

                <div className="border-t border-gray-300">
                    {/* FIRST ITEM */}
                    {items[0] && (
                        <div className="py-6 border-b border-gray-300">
                            <div className="grid grid-cols-[120px_10px_1fr] md:grid-cols-[160px_12px_1fr] text-sm uppercase tracking-widest text-black">
                    ...
                    {items
                        .slice(1)
                        .reduce((rows: any[], item, index, array) => {
                            if (index % 2 === 0) rows.push(array.slice(index, index + 2));
                            return rows;
                        }, [])
                        .map((pair, rowIndex) => (
                            <div key={rowIndex} className="grid grid-cols-1 md:grid-cols-2">
                                {pair.map((item: any, colIndex: number) => (
                                    <div key={colIndex} className="py-6 border-b border-gray-300">
                                        <div className="grid grid-cols-[120px_10px_1fr] md:grid-cols-[160px_12px_1fr] text-sm uppercase tracking-widest text-black">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2">
                        ...
                        <div className="py-6 border-b border-gray-300">
                            <div className="grid grid-cols-[120px_10px_1fr] md:grid-cols-[160px_12px_1fr] text-sm uppercase tracking-widest text-black">
                                <span>Status</span>
                                <span>:</span>
                                <span>{statusData.find((item) => item.value.toString() == status)?.name}</span>
                            </div>
                        </div>
                    </div>
                </div>
