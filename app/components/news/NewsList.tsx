...
const NewsList = ({ data }: { data: NewsData }) => {
  ...
  const years = ["all", ...Array.from(new Set(
    data.news.map(news => new Date(news.date).getFullYear().toString())
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between mt-14 flex-wrap">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              {currentPage > 1 && <div className="flex items-center gap-4">
                <button
                  onClick={() =>
              ...
              <span className="text-sm tracking-widest text-black cursor-pointer transition-colors hover:text-red-500" onClick={() => setCurrentPage(totalPages)}>{"0" + totalPages}</span>
            </div>
            ...
            {currentPage < totalPages && <button
              ...
              className="flex items-center gap-3 text-sm tracking-widest text-red-500 hover:gap-4 transition-all disabled:opacity-40"
            >
              NEXT
            </button>}
          </div>
