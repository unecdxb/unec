    ...
    aboutUsData.awardsData.items.map((item, index) => {
      ...
      return (
        <div key={index} className="relative flex flex-col pb-4 xl:pb-10 transition-all duration-300 group ">
          ...
          <h3 className="text-20 xl:text-25 font-normal leading-[1.2] mb-2 xl:mb-4 uppercase md:max-w-[98%] xl:max-w-[80%] group-hover:text-primary transition-all duration-300">{item.title}</h3>
          <p className="text-16 xl:text-18 text-white/90">{item.subTitle}</p>
        </div>
      );
    })
  }
